...
import cn.huimin100.tc.owf.statemachine.order.state.order.OrderCreateToC;
import cn.huimin100.tc.owf.statemachine.order.state.order.OrderFinish;
import cn.huimin100.tc.owf.statemachine.order.state.pay.PayCancel;
...
public class OrderRequestContext implements RequestContext<Order> {
    ...
    private static final List<OrderStateRequest> ORDER_CREATE_STRATEGIES = new ArrayList<>(
            // TODO 重点配置
            Arrays.asList(new OrderCreate(), new OrderCreateToC()));
    ...
    public OrderRequestContext(Order domain) {
        // 初始化上下文
        System.out.println(Thread.currentThread() + " --- OrderContext <" + this.hashCode() + "> is created! --- ");
        // 初始化状态集 TODO 重点配置 默认订单创建
        this.orderCreate = ORDER_CREATE_STRATEGIES.stream().filter(s -> s.isHandler(domain)).findFirst().orElse(new OrderCreate());
//		this.orderCreate = new OrderCreate();
//		this.orderCreate = new OrderCreateToC();
//		this.orderFinish = new OrderFinish();
//		this.orderCancel = new OrderCancel();
//		this.payCreate = new PayWaiting();
//		this.logisticsCreate = new LogisticsPick();
...
//		StateMachine machine = StateMachine.getInstanceByHash(domain);
        // 刚创建 设置初始状态
        if (domain.getTypeState() == null || domain.getTypeState().get(1) == null
                || domain.getTypeState().get(1).getStateValue() == 0
            /*domain.getState() == null || domain.getState() == 0 || */) {
            // 默认为订单状态
...
//			domain.setTypeState(map);
            ...
            this.state = StateTypeEnum.get(domain);
            this.state.setContext(this);
        }
    }
